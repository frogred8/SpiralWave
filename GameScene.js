...
export class GameScene extends Phaser.Scene {
    ...
    setupPhysics() {
        ...
        this.physics.add.collider(this.resources, this.resources, (obj1, obj2) => {
            ...
            const angle = Phaser.Math.Angle.Between(r1.x, r1.y, obj2.x, obj2.y);
            const pushForce = 150;
            r1.body.velocity.x -= Math.cos(angle) * pushForce;
            r1.body.velocity.y -= Math.sin(angle) * pushForce;
            obj2.body.velocity.x += Math.cos(angle) * pushForce;
            obj2.body.velocity.y += Math.sin(angle) * pushForce;
        });
    ...
    setupUI(skillData) {
        ...
        this.gameStats.once('colorsUnlocked', () => {
            ...
            this.resources.getChildren().forEach(child => child.clearTint());
        });
    ...
    handleInput(pointer) {
        ...
        this.resources.getChildren().forEach(child => {
            const res = child;
            if (!res.active || this.arms.some(a => a.grabbedResource === res))
                return;
        ...
        if (closest)
            arm.fire(closest, this.time.now);
    }
    update(time, delta) {
        ...
        if (this.cursors) {
            ...
            if (this.cursors.left.isDown)
                this.spiralCenter.x -= moveSpeed;
            if (this.cursors.right.isDown)
                this.spiralCenter.x += moveSpeed;
            if (this.cursors.up.isDown)
                this.spiralCenter.y -= moveSpeed;
        ...
        this.whiteHoles.forEach(wh => {
            ...
            if (time > wh.lastSpawnTime + 100) {
                this.createResourceAt(wh.x, wh.y, true);
                wh.lastSpawnTime = time;
            }
        });
        // 로봇팔 업데이트
    ...
    processResources() {
        ...
        this.resources.getChildren().forEach(child => {
            ...
            if (res.itemType === 'special') {
                if (Phaser.Math.Distance.BetweenPoints(res, this.spiralCenter) > 1200)
                    res.destroy();
                return;
            }
    ...
    applyGravity(res, dist, radius) {
        ...
        if (dist < 150)
            res.body.setDrag(50 * res.body.mass);
        else
            res.body.setDrag(0);
    }
    limitSpeed(res, dist) {
        ...
        if (speedSq > max * max)
            ...
            if (speedSq === 0)
                vel.setToPolar(Math.random() * Math.PI * 2, min);
            else
    ...
    handleSpecialItem(item, byArm) {
        if (!byArm) {
            item.destroy();
            this.cameras.main.shake(100, 0.005);
            return;
        }
        if (item.specialType === 'whitehole')
            this.spawnWhiteHole();
        else if (item.specialType === 'boost')
            this.triggerRadiusBoost();
        item.destroy();
        this.cameras.main.shake(100, 0.005);
    }
    triggerRadiusBoost() {
    ...
    spawnSpecialItem() {
        ...
        const item = this.add.text(x, y, type === 'whitehole' ? '🌀' : '⚡', { fontSize: '40px' }).setOrigin(0.5);
    ...
    createResourceAt(x, y, isWhiteHole = false) {
        const types = ['rock', 'wood', 'iron'];
        const type = types[Phaser.Math.Between(0, types.length - 1)];
        const isHighDim = Math.random() < this.gameStats.highDimProb;
        const res = this.add.text(x, y, this.getResourceIcon(type), { fontSize: isHighDim ? '60px' : '24px' }).setOrigin(0.5);
