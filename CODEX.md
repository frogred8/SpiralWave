...
- 커밋 메시지 구성
  ...
  - codex.log 파일에 기록된 시간을 현재 시간과 비교하여 이번 프롬프트에 걸린 작업 시간을 계산.
  - 제목 앞쪽에 [] 사이에 위에서 계산한 작업 시간을 추가.
  - 명령내린 원본 프롬프트를 축약해서 제목 다음 줄에 ()안에 추가.
  - 세부 수정은 그 다음 줄에 내용을 한글로 추가.
  - 아래는 커밋 메시지 예시
  ```
    feat: [1m30s] Bring white hole item to center before spawning outside boundary (CODEX)
    ...
    - 
  ```
- 수정한 내용을 git에 커밋한 이후에 해당 커밋을 codex <codex@openai.com> 이름으로 수동 변경.
