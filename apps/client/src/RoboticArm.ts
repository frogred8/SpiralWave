...
export class RoboticArm {
    ...
    constructor(scene: Phaser.Scene, stats: GameStats, spiralCenter: Phaser.Math.Vector2, onInteract?: (x: number, y: number) => void) {
        this.scene = scene;
        this.stats = stats;
    ...
    private startRetracting(collectCallback: (res: Collectible, byArm: boolean) => void) {
        ...
        this.onInteract?.(this.grabbedResource.x, this.grabbedResource.y);
