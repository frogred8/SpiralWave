export interface SkillData {
    ...
    costs: { rock?: number, wood?: number, iron?: number }[]; // 레벨별 요구 자원 배열
    researchTimes: number[]; // 레벨별 연구 시간 (초)
    description: string;
    prerequisites?: { id: string, level: number }[]; // 다중 선행 조건 시스템
    effectProperty: 'radius' | 'force' | 'highDimProb' | 'maxArms' | 'autoArm' | 'armSpeed' | 'maxResearchSlots' | 'spawnRate' | 'researchBonus' | 'moveSpeed' | 'net'; // 어떤 스탯을 변경할지
